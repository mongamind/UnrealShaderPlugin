...
IMPLEMENT_GLOBAL_SHADER_PARAMETER_STRUCT(FVortexMotorUniformData, "VortexMotorData");

FVortexMotorShader::FVortexMotorShader(const ShaderMetaType::CompiledShaderInitializerType& Initializer)
...
void FVortexMotorShader::ModifyCompilationEnvironment(const FGlobalShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
{
	...
	FShaderResourceViewRHIRef WindDiffusionZAxisTexture_SRV)
{
	...
	if(OutXAxisTexture.IsBound())
	{
		...
		FVortexMotorUniformData UniformData;
		for (int index = 0;index < AllWindMotors.Num();index ++)
		{
			const FVortexMotorParam* MotorParam = AllWindMotors[index];
...
void FVortexMotorShader::UnbindBuffers(FRHICommandListImmediate& RHICmdList)
{
	FRHIComputeShader* ComputeShaderRHI = RHICmdList.GetBoundComputeShader();
	if(OutXAxisTexture.IsBound())
	{
...
void FVortexMotorShader::UnsetParameters(
...
IMPLEMENT_SHADER_TYPE(, FVortexMotorShader, TEXT("/Plugin/WindSimulate/Private/ComputeShader/Motor/VortexMotor.usf"), TEXT("MainCS"), SF_Compute)
